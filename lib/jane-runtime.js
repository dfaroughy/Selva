...
function buildToolCatalog({ extensionPath, workspaceRuntime }) {
  return {
    ...
    dashboardOps: listWebviewActionTools(extensionPath),
...
function buildInstructionPack({ configDir, extensionPath, workspaceRuntime, session }) {
  ...
  const schemata = buildWorkspaceSchemata(workspaceRuntime);
  const dashboardState = clone(activeSession.dashboardState || createDefaultDashboardState());
  const repoContext = readRepoContext(configDir, 2400);
  const schemaBlock = buildSchemaBlock({
    schemata,
    isBootstrap: false,
    fieldCharBudget: 12000,
    dashboardState,
  });
  ...
  const assembledSystemPrompt = buildSystemPrompt({
    ...
    stateBlock,
    allTools,
    additionalPrompt: activeSession.additionalInstructions,
    isBootstrap: false,
  });
  ...
  return {
    ...
    context: {
      repoContext,
      schemaBlock,
      dashboardState: summarizeDashboardState(dashboardState, workspaceRuntime.discoverYamlFiles()),
      sessionSummary: summarizeSessionSnapshot(activeSession, workspaceRuntime.discoverYamlFiles()),
    },
...
async function executeNotebookCellViaTool({
  code,
  configDir,
  extensionPath,
  ...
  const allTools = loadAllTools(extensionPath);
  const pythonTool = allTools.find((tool) => tool.name === 'execute_python' && tool.context === 'extension');
  if (!pythonTool) {
...
async function materializePythonNotebookOutputs(cells, {
  ...
  for (const cell of normalized) {
    ...
    try {
      cell.output = await executeNotebookCellViaTool({
        code: cell.code,
        configDir,
        extensionPath,
        execFileAsync,
...
function buildWorkspaceSchemata(workspaceRuntime) {
  return workspaceRuntime.discoverYamlFiles().map((file) => {
    const { raw, parsed } = workspaceRuntime.readYaml(file);
    return {
      file,
      raw,
...
function createJaneRuntime({ configDir, extensionPath, workspaceRuntime }) {
  ...
  return {
    ...
    async addCells(args = {}, executionOptions = {}) {
      const rawCells = Array.isArray(args.cells) ? args.cells : [];
    ...
    async recordExternalEntry(args = {}, executionOptions = {}) {
      ...
      const isError = !!args.isError;
      const executedCells = Array.isArray(args.executedCells)
        ? args.executedCells
      ...
      const cells = await materializePythonNotebookOutputs(materializeRecordedEntryCells({
        ...
        summary,
        executedCells,
        explicitCells: Array.isArray(args.cells) ? args.cells : undefined,
      }), {
        configDir,
        extensionPath,
        execFileAsync: executionOptions.execFileAsync || defaultExecFileAsync,
