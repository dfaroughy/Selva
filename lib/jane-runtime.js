...
function createJaneRuntime({ configDir, extensionPath, workspaceRuntime }) {
  async function runSessionAction({
    ...
    // Always rebuild schemata from disk (safe summaries only, no raw values).
    // If the webview has unsaved edits, they arrive as a compact pendingEdits
    // delta and are noted in the schema block rather than injected as raw data.
    const resolvedSchemata = buildWorkspaceSchemata(workspaceRuntime);
    ...
    const resolvedPendingEdits = Array.isArray(pendingEdits) ? pendingEdits : [];
    const schemaBlock = buildSchemaBlock({
      schemata: resolvedSchemata,
      isBootstrap,
      fieldCharBudget,
      dashboardState: currentDashboardState,
      pendingEdits: resolvedPendingEdits,
    });
