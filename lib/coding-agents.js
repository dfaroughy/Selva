...
function buildCodingAgentConnectPrompt({ agent, initPayload }) {
  ...
  const lines = [
    ...
    '2. Use `jane_init` as the primary workspace snapshot. Call `jane_get_instruction_pack` only when you need the shared Jane contract or bootstrap prompt text.',
    ...
    '8. Python execution is stateful within the active Trail. Reuse notebook/kernel state when helpful, but reload file-backed data in code instead of pushing large raw datasets into prompt context.',
    '9. Only use `jane_session_run` when Selva has an internal Jane model configured. Otherwise you are Jane and should act through the MCP tools yourself.',
    '',
