...
function buildCodingAgentConnectPrompt({ agent, initPayload, extensionPath, bitacora, taskInstructions }) {
  ...
  const protocol = [
    ...
    '',
    'Initialization:',
    '1. Call `jane_init` to get the workspace snapshot (files, dashboard state, tasks, bitácora).',
    snapshot.needsBootstrap
      ? '2. Bootstrap needed — classify files, set slider bounds, and write the initial Bitácora.'
      : '2. Session exists — review the bitácora and task state before acting.',
    '',
    'Selva tools:',
    '- `read_config`, `set_value`, `get_file_schema` — read and edit YAML configs',
    '- `execute_python` — run Python in the stateful Task kernel',
    '- `jane_add_cells` — add markdown/python/mathjax cells to the notebook',
    ...
    '- `jane_task_new`, `jane_task_fork`, `jane_task_switch` — manage Tasks',
    '- `propose_tool` — create new reusable tools when no built-in tool fits',
    '',
    'Workspace snapshot:',
    `- task: "${snapshot.taskName || 'Task'}" (${snapshot.bootstrapDone ? 'bootstrapped' : 'needs bootstrap'})`,
    ...
    `- other tasks: ${(snapshot.taskCount || 1) - 1}`,
  ].join('\n');

  // ── Task Instructions (human → Jane) ──
  const instructionsSection = taskInstructions
    ? `─── TASK INSTRUCTIONS (from the researcher) ───\n\n${taskInstructions}`
    : '';
