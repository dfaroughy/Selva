...
const { listJaneTasks, loadJaneSession } = require('./session-store');
...
function renderTaskSection(task, session, depth = 0) {
  const entries = session.entries || [];
  const bitacora = session.bitacora || '';
  const headerTag = depth === 0 ? 'h2' : 'h3';

  let html = `<section class="task" id="task-${escapeHtml(task.id)}">`;
  html += `<${headerTag} class="task-title">${escapeHtml(task.name)}</${headerTag}>`;

  if (task.parentTaskId) {
    html += `<p class="task-meta">Forked from parent task</p>`;
  }
  if (task.createdAt) {
    html += `<p class="task-meta">Started: ${new Date(task.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>`;
  }
...
function buildTaskTree(tasks) {
  // Build a tree from flat task list using parentTaskId
...
function renderProjectGraph(tasks) {
  // Simple text representation of the task tree for the HTML export
  const tree = buildTaskTree(tasks);
  if (tree.length === 0) return '';

  let html = `<div class="project-graph"><h3>Research Graph</h3><pre class="graph-tree">`;
  for (const { task, depth } of tree) {
    const indent = '  '.repeat(depth);
    const connector = depth > 0 ? '└─ ' : '';
    const status = task.active ? ' ●' : '';
    html += `${indent}${connector}${escapeHtml(task.name)}${status}\n`;
  }
  html += `</pre></div>`;
  return html;
}
...
const CSS = `
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .task:first-of-type { page-break-before: auto; }
  .task-title { color: #1a6b3a; }
  .task-meta { font-size: 9.5pt; color: #888; margin: 2px 0; }
...
function exportProjectToHtml(configDir, options = {}) {
  ...
  const tasks = listJaneTasks(configDir);
  ...
  body += `<p class="project-meta">Exported from Selva on ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })} &mdash; ${tasks.length} task${tasks.length !== 1 ? 's' : ''}</p>`;
  ...
  // Task graph
  body += renderProjectGraph(tasks);

  // Each task
  for (const { task, depth } of taskTree) {
    const session = loadJaneSession(configDir, task.id);
    body += renderTaskSection(task, session, depth);
  }
