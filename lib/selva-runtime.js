...
const BUILTIN_MCP_TOOLS = Object.freeze([
  ...
  {
    name: 'approve_tool',
    description: 'Approve a user-created tool for execution. Tools created via propose_tool require approval before they can be called.',
    inputSchema: {
      type: 'object',
      properties: {
        name: { type: 'string', description: 'Tool name to approve' },
      },
      required: ['name'],
    },
  },
...
const SCHEMA_LARGE_OBJECT_THRESHOLD = 20;
...
function buildSchema(file, parsed) {
  ...
  function walk(obj, pathArr, depth) {
    if (obj == null || typeof obj !== 'object') return;
    if (fields.length >= SCHEMA_MAX_FIELDS) return;

    const keys = Array.isArray(obj) ? obj.map((_, idx) => idx) : Object.keys(obj);

    // Summarize large objects or deep nesting as a single field instead of recursing.
    if (!Array.isArray(obj) && (depth >= SCHEMA_MAX_DEPTH || keys.length >= SCHEMA_LARGE_OBJECT_THRESHOLD)) {
      fields.push({
        path: pathArr,
  ...
  if (fields.length >= SCHEMA_MAX_FIELDS) {
    fields.push({
      path: ['...'],
      preview: `(truncated at ${SCHEMA_MAX_FIELDS} fields — use get_file_schema or execute_python for full inspection)`,
      type: 'note',
    });
  }
...
function createWorkspaceRuntime(options) {
  ...
  async function callTool(name, args = {}) {
    switch (name) {
      ...
      case 'approve_tool':
        return approveToolByName(args.name);

      default: {
        if (dynamicTools.has(name)) {
          const dynTool = dynamicTools.get(name);
          if (dynTool.source === USER_TOOLS_DIR && !isToolApproved(dynTool.toolDir)) {
            return `Tool "${name}" has not been approved for execution. Use approve_tool to approve it.`;
          }
          try {
