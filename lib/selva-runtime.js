...
const { fixPythonForHeadless } = require('./notebook-python');
const { loadJaneSession } = require('./session-store');
const { coerceValue } = require('./value-coerce');
...
const BLOCKED_CODE_PATTERNS = [
  ...
  { pattern: /\brequire\s*\(\s*['"]net['"]\s*\)/, label: "require('net')" },
  { pattern: /\brequire\s*\(\s*['"]http['"]\s*\)/, label: "require('http')" },
  { pattern: /\brequire\s*\(\s*['"]https['"]\s*\)/, label: "require('https')" },
...
function validateToolCode(code) {
  ...
  for (const { pattern, label } of BLOCKED_CODE_PATTERNS) {
    if (pattern.test(code)) {
      errors.push(`Blocked pattern: ${label}`);
    }
  }
  return errors;
}

function isToolApproved(toolDir) {
  try {
    const meta = JSON.parse(fs.readFileSync(path.join(toolDir, 'metadata.json'), 'utf8'));
...
function approveToolByName(name) {
  ...
  return `Tool "${name}" approved for execution.`;
}
...
const BUILTIN_MCP_TOOLS = Object.freeze([
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        taskId: {
          type: 'string',
          description: 'Optional Task id for stateful execution. Defaults to the active Task when available.',
        },
        input_data: {
          type: 'object',
  ...
  {
    name: 'propose_tool',
    description: 'Create a new reusable MCP tool in the Selva ecosystem (~/.selva/ecosystem/tools/). Write Python code for the tool body. The tool persists across sessions and is immediately available for use.',
    inputSchema: {
      type: 'object',
      properties: {
...
const SUPPORTED_FILE_PATTERN = /\.(ya?ml|json)$/i;
...
function discoverYamlFiles(dir, relativePrefix = '') {
  ...
  try {
    const entries = fs.readdirSync(dir, { withFileTypes: true });
    for (const entry of entries) {
      if (entry.isFile() && SUPPORTED_FILE_PATTERN.test(entry.name)) {
...
function readYamlFile(configDir, file) {
  ...
  if (isJsonFile(file)) {
    return { full, raw, parsed: JSON.parse(raw) };
  }
...
const SCHEMA_MAX_FIELDS = 500;
const SCHEMA_INITIAL_MAX_DEPTH = 6;
const SCHEMA_MIN_DEPTH = 2;
const SCHEMA_LARGE_OBJECT_THRESHOLD = 50;
...
function buildSchema(file, parsed) {
  ...
  function summarizeObjectKeys(obj) {
    const keys = Object.keys(obj);
    const sampleKeys = keys.slice(0, 5).join(',');
    const sampleVals = [];
    for (const k of keys.slice(0, 3)) {
      sampleVals.push(classifyValueType(obj[k]));
    }
    const valTypes = [...new Set(sampleVals)].join('|');
    return `object(keys=${keys.length}, sample={${sampleKeys}}, vals=${valTypes})`;
  }
  ...
  function summarizeValue(value) {
    ...
    if (kind === 'object') {
      return summarizeObjectKeys(value);
    }
    if (kind === 'string') {
  ...
  function walk(obj, pathArr, depth, maxDepth, fields) {
    if (obj == null || typeof obj !== 'object') return;
    if (fields.length >= SCHEMA_MAX_FIELDS) return;

    const keys = Array.isArray(obj) ? obj.map((_, idx) => idx) : Object.keys(obj);

    if (!Array.isArray(obj) && (depth >= maxDepth || keys.length >= SCHEMA_LARGE_OBJECT_THRESHOLD)) {
      fields.push({
        path: pathArr,
        preview: summarizeObjectKeys(obj),
        type: 'object',
      });
      return;
    }
    ...
    for (const key of keys) {
      if (fields.length >= SCHEMA_MAX_FIELDS) break;
      const val = obj[key];
      const nextPath = [...pathArr, String(key)];
      if (val != null && typeof val === 'object' && !Array.isArray(val)) {
        walk(val, nextPath, depth + 1, maxDepth, fields);
      } else {
        fields.push({
          path: nextPath,
  ...
  // Adaptive depth: try deepest first, reduce if too many fields
  let fields = [];
  let effectiveDepth = SCHEMA_INITIAL_MAX_DEPTH;

  for (let depth = SCHEMA_INITIAL_MAX_DEPTH; depth >= SCHEMA_MIN_DEPTH; depth--) {
    fields = [];
    walk(parsed, [], 0, depth, fields);
  ...
  if (fields.length >= SCHEMA_MAX_FIELDS) {
    fields.push({
      path: ['...'],
      preview: `(truncated at ${SCHEMA_MAX_FIELDS} fields, depth ${effectiveDepth} — use get_file_schema or execute_python for full inspection)`,
      type: 'note',
    });
  }
...
function createWorkspaceRuntime(options) {
  ...
  async function callSessionTool(toolDef, args) {
    ...
    const activeTaskId = String((loadJaneSession(configDir).taskId || ''));
    return handler(args, {
      execFileAsync,
      configDir,
      taskId: String(args && args.taskId ? args.taskId : activeTaskId),
    });
  }

  async function proposeTool(args) {
    const codeErrors = validateToolCode(args.code);
    if (codeErrors.length > 0) {
      return 'Tool code rejected:\n' + codeErrors.map((e) => '  - ' + e).join('\n');
    }
    ...
    switch (name) {
      case 'list_files': {
        ...
        return files.length
          ...
          : 'No YAML or JSON files found in ' + configDir;
      }
      ...
      default: {
        if (dynamicTools.has(name)) {
          const dynTool = dynamicTools.get(name);
          try {
            return await callSessionTool(dynTool, args);
          } catch (err) {
            return 'Error executing tool ' + name + ': ' + err.message;
          }
        }
        const userToolDir = path.join(USER_TOOLS_DIR, name);
        const userToolPath = path.join(userToolDir, 'tool.js');
