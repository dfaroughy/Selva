...
const BUILTIN_MCP_TOOLS = Object.freeze([
  ...
  {
    name: 'read_config',
    description: 'Read and parse a YAML config file. Returns field paths, values, types, and raw YAML.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Get the full schema (all field paths, values, types) and raw YAML for a file. Use to inspect before editing.',
    inputSchema: {
      type: 'object',
      properties: {
        file: { type: 'string', description: 'YAML filename' },
...
function createWorkspaceRuntime(options) {
  ...
  async function callSessionTool(toolDef, args) {
    ...
    return handler(args, { execFileAsync, configDir });
  }
  ...
  async function callTool(name, args = {}) {
    switch (name) {
      ...
      case 'read_config': {
        const { raw, parsed } = readYamlFile(configDir, args.file);
      ...
      case 'execute_python':
        return executePython(configDir, args, execFileAsync);
      ...
      case 'get_file_schema': {
        ...
        const lines = schema.fields
          ...
          .join('\n');
        let result = `[${args.file}]\nFIELDS (${schema.fields.length} total):\n${lines}`;
        const rawPreview = raw.slice(0, 3000);
        result += '\n\nRAW YAML:\n' + rawPreview;
        if (raw.length > 3000) result += '\n... (truncated, ' + raw.length + ' chars total)';
        return result;
      }
