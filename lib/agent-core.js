...
function buildSchemaBlock({ schemata = [], isBootstrap = false, fieldCharBudget = 0, dashboardState = {}, pendingEdits = [] }) {
  if (isBootstrap) {
    ...
    return schemata.map(({ file, fields }) => {
      ...
      const lines = fields.slice(0, budget).map(({ path, preview, type }) =>
        `    ${JSON.stringify(path)}  =  ${String(preview || type)}  (${type})`
      ).join('\n');
  ...
  let block = schemata.map(({ file, fields }) => {
  ...
  if (pendingEdits && pendingEdits.length > 0) {
    const editLines = pendingEdits.map((e) =>
      `  ${e.file}:${(e.path || []).join('.')} = ${JSON.stringify(e.value)}`
    ).join('\n');
    block += `\n\nUNSAVED EDITS (${pendingEdits.length}, not yet written to disk):\n${editLines}`;
  }
...
function compactToolResult(result) {
  ...
  // Keep the first and last portions so the model sees the beginning (headers,
  // setup messages) and the end (final status, errors, summaries).
  ...
  const dropped = result.length - headBudget - tailBudget;

  return head
    + `\n\n... (${dropped} chars omitted — full output is in the notebook cell) ...\n\n`
    + tail;
}
...
async function executeToolCall(call, options) {
  ...
  try {
    if (tool.context === 'extension') {
      ...
      result = captureImagesFromResult(result, capturedImages);
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    for (const toolCall of toolCalls) {
      ...
      try {
        ...
        apiMessages.push({ role: 'tool', tool_call_id: toolCall.id, content: 'Error: malformed tool arguments: ' + parseError.message });
        continue;
      }
