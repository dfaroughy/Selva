...
class PythonNotebookKernel {
  constructor({ configDir, trailId, language = 'python', spawnImpl = spawn, executeTimeoutMs = DEFAULT_EXECUTE_TIMEOUT_MS }) {
...
class NotebookKernelManager {
  ...
  getOrCreateRuntime({ language, configDir, trailId, executeTimeoutMs }) {
    ...
    const opts = { configDir, trailId, language };
