...
function getArtifactsDir(configDir, trailId) {
  return path.join(getTrailDir(configDir, trailId), 'artifacts');
}

function externalizeArtifacts(configDir, trailId, notebook) {
  ...
  const artifactsDir = getArtifactsDir(configDir, trailId);
  let artifactSeq = 0;
...
function internalizeArtifacts(configDir, trailId, notebook) {
...
function mergeSessionFromFiles(trailMeta, notebook, context, configDir, meta) {
  ...
  const trailId = (trailMeta && trailMeta.trailId) || (meta && meta.trailId) || '';
  if (notebook && trailId) {
    internalizeArtifacts(configDir, trailId, notebook);
  }
...
function _saveTrailSessionInner(configDir, trailId, session, options = {}) {
  ...
  externalizeArtifacts(configDir, requestedTrailId, notebook);
...
function createJaneTrail(configDir, options = {}) {
  ...
  const fresh = createEmptyTrailSession(configDir, {
    ...
    bitacora: '',  // New trails start with empty bitácora — fresh line of inquiry
  });
