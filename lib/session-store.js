const fs = require('fs');
const os = require('os');
const path = require('path');
const crypto = require('crypto');
const { JUNGLE_A, JUNGLE_B } = require('../ecosystem/tools/propose_tool/bigrams.js');
const { withFileLockSync } = require('./file-lock');

const WORKSPACE_STORE_DIRNAME = '.selva';
const TASKS_DIRNAME = 'tasks';
const TASK_INDEX_FILENAME = 'index.json';
const TASK_FILE_EXTENSION = '.svnb';
const TASK_CONTEXT_EXTENSION = '.svctx';

function ensureDir(dirPath) {
  fs.mkdirSync(dirPath, { recursive: true });
}

function clone(value) {
  return JSON.parse(JSON.stringify(value));
}

function createEntityId(prefix = 'entity') {
  return `${prefix}_${crypto.randomUUID().replace(/-/g, '')}`;
}

function createDefaultDashboardState() {
  return {
    fileTypes: {},
    lockedFields: [],
    pinnedFields: {},
    activeConfigFile: null,
    activeDataFile: null,
  };
}

function createDefaultPanelState() {
  return {
    open: false,
    updatedAt: null,
  };
}

function createDefaultJaneSession(configDir) {
  return {
    version: 1,
    configDir,
    conversationHistory: [],
    entries: [],
    additionalInstructions: '',
    bitacora: '',
    agentModelId: '',
    sessionTokens: 0,
    lastQuestion: '',
    dashboardState: createDefaultDashboardState(),
    bootstrap: {
      done: false,
      answer: '',
      ops: [],
    },
    sessionSummary: '',  // Compressed summary of older conversation entries
    panelState: createDefaultPanelState(),
    pendingExternalDrafts: [],
    updatedAt: new Date().toISOString(),
  };
}

function workspaceIdForConfigDir(configDir) {
  return crypto.createHash('sha1').update(String(configDir)).digest('hex');
}

function sessionIdForConfigDir(configDir) {
  return workspaceIdForConfigDir(configDir);
}

function getWorkspaceStoreDir(configDir) {
  return path.join(configDir, WORKSPACE_STORE_DIRNAME);
}

function getTasksDir(configDir) {
  const tasksDir = path.join(getWorkspaceStoreDir(configDir), TASKS_DIRNAME);
  // Backward compat: if 'tasks/' doesn't exist but 'trails/' does, use 'trails/'
  if (!fs.existsSync(tasksDir)) {
    const legacyDir = path.join(getWorkspaceStoreDir(configDir), 'trails');
    if (fs.existsSync(legacyDir)) return legacyDir;
  }
  ensureDir(tasksDir);
  return tasksDir;
}

function getTaskIndexPath(configDir) {
  return path.join(getTasksDir(configDir), TASK_INDEX_FILENAME);
}

function slugify(name) {
  return String(name || 'task').trim()
    .replace(/\s+/g, '_')
    .replace(/[^A-Za-z0-9_\-]/g, '')
    .replace(/_+/g, '_')
    .replace(/^_+|_+$/g, '') || 'task';
}

// Core path functions take dirName (slugified task name).
// NOTE: _taskDir does NOT create the directory. Directories are created
// only when writing files (via writeJsonAtomic which calls ensureDir on the parent).
function _taskDir(configDir, dirName) {
  return path.join(getTasksDir(configDir), dirName);
}
function _taskPath(configDir, dirName) {
  return path.join(_taskDir(configDir, dirName), `${dirName}${TASK_FILE_EXTENSION}`);
}
function _taskContextPath(configDir, dirName) {
  return path.join(_taskDir(configDir, dirName), `${dirName}${TASK_CONTEXT_EXTENSION}`);
}
function _taskMetaPath(configDir, dirName) {
  return path.join(_taskDir(configDir, dirName), 'task.json');
}

// Public path functions accept taskId and resolve to dirName via index.
function getTaskDir(configDir, taskId) {
  return _taskDir(configDir, dirNameForTaskId(configDir, taskId));
}
function getTaskPath(configDir, taskId) {
  return _taskPath(configDir, dirNameForTaskId(configDir, taskId));
}
function getTaskContextPath(configDir, taskId) {
  return _taskContextPath(configDir, dirNameForTaskId(configDir, taskId));
}
function getTaskMetaPath(configDir, taskId) {
  return _taskMetaPath(configDir, dirNameForTaskId(configDir, taskId));
}
function taskLockPath(configDir, taskId) {
  return getTaskPath(configDir, taskId) + '.lock';
}

// Look up dirName from taskId via the index (cached per configDir)
const _dirNameCache = new Map();
function dirNameForTaskId(configDir, taskId) {
  const cacheKey = `${configDir}:${taskId}`;
  if (_dirNameCache.has(cacheKey)) return _dirNameCache.get(cacheKey);
  const index = readJsonFile(getTaskIndexPath(configDir));
  if (index && Array.isArray(index.tasks)) {
    // Cache all tasks at once
    for (const t of index.tasks) {
      const dn = t.dirName || slugify(t.name || 'task');
      _dirNameCache.set(`${configDir}:${t.id}`, dn);
    }
    const cached = _dirNameCache.get(cacheKey);
    if (cached) return cached;
  }
  return taskId;
}
function invalidateDirNameCache(configDir) {
  for (const key of _dirNameCache.keys()) {
    if (key.startsWith(configDir + ':')) _dirNameCache.delete(key);
  }
}

function createRandomTaskBase(hashHex = crypto.randomBytes(8).toString('hex')) {
  const a = parseInt(hashHex.slice(0, 2), 16) % JUNGLE_A.length;
  const b = parseInt(hashHex.slice(2, 4), 16) % JUNGLE_B.length;
  return `${JUNGLE_A[a]} ${JUNGLE_B[b]}`;
}

function createDefaultTaskName(existingCount = 0, existingNames = []) {
  const used = new Set(Array.isArray(existingNames) ? existingNames : []);
  for (let i = 0; i < 16; i += 1) {
    const candidate = createRandomTaskBase();
    if (!used.has(candidate)) return candidate;
  }
  return `${createRandomTaskBase()} ${existingCount + 1}`;
}

function createTaskIdentity(taskName) {
  const bigram = createRandomTaskBase();
  const hex = crypto.randomBytes(8).toString('hex');
  const taskId = `${bigram.replace(/\s+/g, '_')}_${hex}`;
  // If no name provided, reuse the bigram as the human-readable name
  const name = String(taskName || '').trim() || bigram;
  return { taskId, taskName: name };
}

function createTaskId() {
  return createTaskIdentity('').taskId;
}

function normalizeDraftOp(op) {
  return {
    fn: String(op.fn || ''),
    input: clone(op.input || {}),
  };
}

function normalizeExternalDraft(draft) {
  return {
    id: String(draft.id || crypto.randomUUID()),
    source: String(draft.source || 'external'),
    note: String(draft.note || ''),
    createdAt: draft.createdAt || new Date().toISOString(),
    ops: Array.isArray(draft.ops) ? draft.ops.map(normalizeDraftOp) : [],
  };
}

function normalizeExecutedCell(cell) {
  return {
    code: String(cell && cell.code ? cell.code : ''),
    output: String(cell && cell.output ? cell.output : ''),
  };
}

function isLegacyPendingPythonOutput(output) {
  const text = String(output || '').trim();
  if (!text) return false;
  // Compatibility for notebook cells persisted before explicit runState existed.
  return /^\[[^\]]*\bplot\b[^\]]*\]$/i.test(text);
}

function looksLikePythonExecutionError(output) {
  const text = String(output || '').trim();
  if (!text) return false;
  return /^Error \(exit\s+\d+\):/i.test(text)
    || /^Execution error:/i.test(text)
    || /Traceback \(most recent call last\):/i.test(text)
    || /\b(?:SyntaxError|NameError|TypeError|ValueError|IndexError|KeyError|ModuleNotFoundError|ImportError|AttributeError|RuntimeError)\b/.test(text);
}

function normalizePythonRunState(runState, output) {
  const normalized = String(runState || '').trim().toLowerCase();
  if (normalized === 'pending' || normalized === 'done' || normalized === 'error') {
    return normalized;
  }
  if (normalized === 'running') {
    return 'pending';
  }

  const text = String(output || '').trim();
  if (!text) return 'pending';
  if (isLegacyPendingPythonOutput(text)) return 'pending';
  if (looksLikePythonExecutionError(text)) return 'error';
  return 'done';
}

function normalizeCellProvenance(cell) {
  const author = (cell.author === 'human' || cell.author === 'user') ? 'human' : 'jane';
  const result = { author };
  if (cell.editedBy === 'human' || cell.editedBy === 'jane') {
    result.editedBy = cell.editedBy;
  }
  if (cell.createdAt) result.createdAt = cell.createdAt;
  if (cell.editedAt) result.editedAt = cell.editedAt;
  return result;
}

function normalizeNotebookCell(cell) {
  if (!cell || typeof cell !== 'object') return null;
  const type = String(cell.type || '').toLowerCase();
  if (!type) return null;
  const id = String(cell.id || createEntityId('cell'));
  const provenance = normalizeCellProvenance(cell);

  if (type === 'markdown') {
    return {
      id,
      type: 'markdown',
      content: String(cell.content || ''),
      ...provenance,
    };
  }

  if (type === 'python') {
    return {
      id,
      type: 'python',
      code: String(cell.code || ''),
      output: String(cell.output || ''),
      runState: normalizePythonRunState(cell.runState, cell.output),
      ...provenance,
    };
  }

  if (type === 'image') {
    return {
      id,
      type: 'image',
      data: String(cell.data || ''),
      ...provenance,
    };
  }

  if (type === 'diff') {
    return {
      id,
      type: 'diff',
      diffs: Array.isArray(cell.diffs) ? clone(cell.diffs) : [],
      ...provenance,
    };
  }

  return {
    id,
    type,
    lang: String(cell.lang || ''),
    content: String(cell.content || ''),
    ...provenance,
  };
}

function normalizeSessionEntry(entry) {
  const rawExecuted = Array.isArray(entry && entry.executedCells)
    ? entry.executedCells.map(normalizeExecutedCell).filter((cell) => cell.code)
    : [];

  const normalized = {
    id: String(entry && entry.id ? entry.id : createEntityId('entry')),
    question: String(entry && entry.question ? entry.question : ''),
    answer: String(entry && entry.answer ? entry.answer : ''),
    summary: String(entry && entry.summary ? entry.summary : ''),
    isError: !!(entry && entry.isError),
    timestamp: entry && entry.timestamp ? entry.timestamp : new Date().toISOString(),
  };

  // Always present in the in-memory model for API compat; stripped from .svnb v2 on save
  normalized.executedCells = rawExecuted;

  if (Array.isArray(entry && entry.cells)) {
    normalized.cells = entry.cells
      .map(normalizeNotebookCell)
      .filter((cell) => cell && (
        (cell.type === 'markdown' && cell.content)
        || (cell.type === 'python' && cell.code)
        || (cell.type === 'image' && cell.data)
        || (cell.type === 'diff')
        || cell.content
      ));
  }

  return normalized;
}

const MAX_CONVERSATION_HISTORY_ENTRIES = 200;
const MAX_CONVERSATION_HISTORY_CHARS = 500000;
const SUMMARY_MAX_CHARS = 5000;

function capConversationHistory(history) {
  if (!Array.isArray(history)) return { capped: [], dropped: [] };
  let capped = history.slice(-MAX_CONVERSATION_HISTORY_ENTRIES);
  const droppedByCount = history.slice(0, Math.max(0, history.length - MAX_CONVERSATION_HISTORY_ENTRIES));

  let totalChars = capped.reduce((sum, turn) => sum + (turn.content || '').length, 0);
  const droppedBySize = [];
  while (totalChars > MAX_CONVERSATION_HISTORY_CHARS && capped.length > 2) {
    totalChars -= (capped[0].content || '').length;
    droppedBySize.push(capped[0]);
    capped = capped.slice(1);
  }
  if (capped.length > 0 && capped[0].role !== 'user') {
    droppedBySize.push(capped[0]);
    capped = capped.slice(1);
  }
  return { capped, dropped: [...droppedByCount, ...droppedBySize] };
}

function buildConversationSummary(existingSummary, droppedEntries) {
  if (!droppedEntries || droppedEntries.length === 0) return existingSummary || '';

  const newSummaryParts = [];
  for (const turn of droppedEntries) {
    const content = String(turn.content || '').trim();
    if (!content) continue;
    const role = turn.role === 'user' ? 'User' : 'Assistant';
    // Take first 200 chars of each dropped turn
    const preview = content.length > 200 ? content.slice(0, 200) + '...' : content;
    newSummaryParts.push(`[${role}]: ${preview}`);
  }

  const newSummaryText = newSummaryParts.join('\n');
  const combined = existingSummary
    ? existingSummary + '\n---\n' + newSummaryText
    : newSummaryText;

  // Cap the summary itself
  if (combined.length > SUMMARY_MAX_CHARS) {
    return combined.slice(-SUMMARY_MAX_CHARS);
  }
  return combined;
}

function normalizeSession(session, configDir, meta = {}) {
  const base = createDefaultJaneSession(configDir);
  const fallbackTaskName = String(meta.taskName || session.taskName || 'Task');
  const taskId = String(meta.taskId || session.taskId || createTaskId());
  const createdAt = meta.createdAt || session.createdAt || new Date().toISOString();
  const taskName = fallbackTaskName;

  const cappedResult = capConversationHistory(Array.isArray(session.conversationHistory) ? session.conversationHistory : []);
  const sessionSummary = buildConversationSummary(
    String(session.sessionSummary || ''),
    cappedResult.dropped
  );

  return {
    ...base,
    ...session,
    configDir,
    taskId,
    taskName,
    createdAt,
    dashboardState: {
      ...base.dashboardState,
      ...(session.dashboardState || {}),
      fileTypes: { ...(base.dashboardState.fileTypes || {}), ...((session.dashboardState || {}).fileTypes || {}) },
      pinnedFields: { ...(base.dashboardState.pinnedFields || {}), ...((session.dashboardState || {}).pinnedFields || {}) },
      lockedFields: Array.isArray((session.dashboardState || {}).lockedFields)
        ? [...session.dashboardState.lockedFields]
        : [...base.dashboardState.lockedFields],
    },
    bootstrap: {
      ...base.bootstrap,
      ...(session.bootstrap || {}),
      ops: Array.isArray((session.bootstrap || {}).ops) ? [...session.bootstrap.ops] : [],
    },
    panelState: {
      ...base.panelState,
      ...(session.panelState || {}),
    },
    bitacora: String(session.bitacora || ''),
    conversationHistory: cappedResult.capped,
    sessionSummary,
    entries: Array.isArray(session.entries) ? session.entries.map(normalizeSessionEntry) : [],
    pendingExternalDrafts: Array.isArray(session.pendingExternalDrafts)
      ? session.pendingExternalDrafts.map(normalizeExternalDraft).filter((draft) => draft.ops.length > 0)
      : [],
  };
}

function readJsonFile(filePath) {
  try {
    return JSON.parse(fs.readFileSync(filePath, 'utf8'));
  } catch {
    return null;
  }
}

function writeJsonAtomic(filePath, value) {
  ensureDir(path.dirname(filePath));
  const tmpPath = filePath + '.tmp';
  fs.writeFileSync(tmpPath, JSON.stringify(value, null, 2), 'utf8');
  fs.renameSync(tmpPath, filePath);
}

function summarizeTaskSession(session) {
  return {
    id: String(session.taskId || ''),
    name: String(session.taskName || 'Task'),
    dirName: slugify(String(session.taskName || 'Task')),
    parentTaskId: String(session.parentTaskId || ''),
    createdAt: session.createdAt || session.updatedAt || new Date().toISOString(),
    updatedAt: session.updatedAt || session.createdAt || new Date().toISOString(),
    bootstrapDone: !!(session.bootstrap && session.bootstrap.done),
    entryCount: Array.isArray(session.entries) ? session.entries.length : 0,
    lastQuestion: session.lastQuestion || '',
  };
}

function normalizeTaskRecord(record) {
  const recordName = String(record && record.name ? record.name : 'Task');
  const recordId = String(record && record.id ? record.id : createTaskId());
  const dirName = String(record && record.dirName ? record.dirName : slugify(recordName));
  return {
    id: recordId,
    name: recordName,
    dirName,
    parentTaskId: String(record && record.parentTaskId ? record.parentTaskId : ''),
    createdAt: record && record.createdAt ? record.createdAt : new Date().toISOString(),
    updatedAt: record && record.updatedAt ? record.updatedAt : new Date().toISOString(),
    bootstrapDone: !!(record && record.bootstrapDone),
    entryCount: Number(record && record.entryCount ? record.entryCount : 0),
    lastQuestion: String(record && record.lastQuestion ? record.lastQuestion : ''),
  };
}

function normalizeTaskIndex(index, configDir) {
  return {
    version: 1,
    configDir,
    workspaceId: workspaceIdForConfigDir(configDir),
    activeTaskId: index && index.activeTaskId ? String(index.activeTaskId) : '',
    tasks: Array.isArray(index && index.tasks) ? index.tasks.map(normalizeTaskRecord) : [],
  };
}

function createEmptyTaskSession(configDir, options = {}) {
  const base = createDefaultJaneSession(configDir);
  if (options.agentModelId) base.agentModelId = options.agentModelId;
  if (options.additionalInstructions) base.additionalInstructions = options.additionalInstructions;
  if (options.bitacora) base.bitacora = options.bitacora;
  if (options.panelOpen) {
    base.panelState = {
      ...createDefaultPanelState(),
      open: true,
      updatedAt: new Date().toISOString(),
    };
  }
  return normalizeSession(base, configDir, {
    taskId: options.taskId,
    taskName: options.taskName,
    createdAt: options.createdAt,
  });
}

function ensureTaskStore(configDir) {
  const indexPath = getTaskIndexPath(configDir);
  const existing = readJsonFile(indexPath);

  if (!existing || !Array.isArray(existing.tasks)) {
    // New project: empty task store — user creates the first task
    const index = normalizeTaskIndex({ activeTaskId: '', tasks: [] }, configDir);
    writeJsonAtomic(indexPath, index);
    return index;
  }

  const normalized = normalizeTaskIndex(existing, configDir);
  let changed = false;

  for (const task of normalized.tasks) {
    const taskMetaExists = fs.existsSync(getTaskMetaPath(configDir, task.id));
    const taskNbExists = fs.existsSync(getTaskPath(configDir, task.id));

    if (!taskNbExists && !taskMetaExists) {
      const restored = createEmptyTaskSession(configDir, {
        taskId: task.id,
        taskName: task.name,
        createdAt: task.createdAt,
      });
      const { taskMeta: tm, notebook: nb, context: ctx } = splitSessionForPersistence(restored);
      writeJsonAtomic(getTaskMetaPath(configDir, task.id), tm);
      writeJsonAtomic(getTaskPath(configDir, task.id), nb);
      writeJsonAtomic(getTaskContextPath(configDir, task.id), ctx);
      Object.assign(task, summarizeTaskSession(restored));
      changed = true;
      continue;
    }

    const taskMetaFile = readJsonFile(getTaskMetaPath(configDir, task.id));
    const notebookFile = readJsonFile(getTaskPath(configDir, task.id));
    const source = taskMetaFile || notebookFile;
    if (source) {
      const merged = taskMetaFile
        ? { ...source, entries: (notebookFile && notebookFile.entries) || [] }
        : source;
      Object.assign(task, summarizeTaskSession(normalizeSession(merged, configDir, {
        taskId: task.id,
        taskName: task.name,
        createdAt: task.createdAt,
      })));
    }
  }

  if (normalized.tasks.length > 0 && (!normalized.activeTaskId || !normalized.tasks.some((t) => t.id === normalized.activeTaskId))) {
    normalized.activeTaskId = normalized.tasks[0].id;
    changed = true;
  }
  if (normalized.tasks.length === 0) {
    normalized.activeTaskId = '';
    changed = true;
  }

  if (changed) {
    writeJsonAtomic(indexPath, normalized);
  }
  return normalized;
}

function listJaneTasks(configDir) {
  const index = ensureTaskStore(configDir);
  return index.tasks.map((task) => ({
    ...task,
    active: task.id === index.activeTaskId,
    path: getTaskPath(configDir, task.id),
  }));
}

function getActiveTask(configDir) {
  const index = ensureTaskStore(configDir);
  const task = index.tasks.find((item) => item.id === index.activeTaskId) || index.tasks[0] || null;
  if (!task) return null;
  return {
    ...task,
    active: true,
    path: getTaskPath(configDir, task.id),
  };
}

function getSessionPath(configDir) {
  const task = getActiveTask(configDir);
  if (!task) {
    const { taskId } = createTaskIdentity(createDefaultTaskName(0));
    return getTaskPath(configDir, taskId);
  }
  return task.path;
}

function loadJaneSession(configDir, taskId = null) {
  const index = ensureTaskStore(configDir);
  const target = taskId
    ? (index.tasks.find((task) => task.id === String(taskId)) || null)
    : (index.tasks.find((task) => task.id === index.activeTaskId) || index.tasks[0] || null);

  if (!target) {
    // No tasks exist — return a bare default session without writing anything to disk
    return createDefaultJaneSession(configDir);
  }

  const meta = {
    taskId: target.id,
    taskName: target.name,
    createdAt: target.createdAt,
  };

  // Try reading task.json (v2 three-file format)
  const taskMetaFile = readJsonFile(getTaskMetaPath(configDir, target.id));
  const notebookFile = readJsonFile(getTaskPath(configDir, target.id));
  const contextFile = readJsonFile(getTaskContextPath(configDir, target.id));

  // v1 migration: .svnb contains everything (conversationHistory, entries, dashboard, etc.)
  if (!taskMetaFile && notebookFile && (notebookFile.version !== 2 || notebookFile.dashboardState || Array.isArray(notebookFile.conversationHistory))) {
    const migrated = normalizeSession(notebookFile, configDir, meta);
    const { taskMeta: tm, notebook: nb, context: ctx } = splitSessionForPersistence(migrated);
    writeJsonAtomic(getTaskMetaPath(configDir, target.id), tm);
    writeJsonAtomic(getTaskPath(configDir, target.id), nb);
    writeJsonAtomic(getTaskContextPath(configDir, target.id), ctx);
    return migrated;
  }

  // No files at all — create fresh
  if (!taskMetaFile && !notebookFile) {
    const restored = createEmptyTaskSession(configDir, {
      taskId: target.id,
      taskName: target.name,
      createdAt: target.createdAt,
    });
    const { taskMeta: tm, notebook: nb, context: ctx } = splitSessionForPersistence(restored);
    writeJsonAtomic(getTaskMetaPath(configDir, target.id), tm);
    writeJsonAtomic(getTaskPath(configDir, target.id), nb);
    writeJsonAtomic(getTaskContextPath(configDir, target.id), ctx);
    return restored;
  }

  // v2: merge from three files
  return mergeSessionFromFiles(taskMetaFile, notebookFile, contextFile, configDir, meta);
}

function upsertTaskMeta(index, session) {
  const nextMeta = summarizeTaskSession(session);
  const idx = index.tasks.findIndex((task) => task.id === nextMeta.id);
  if (idx >= 0) index.tasks[idx] = nextMeta;
  else index.tasks.push(nextMeta);
}

const ARTIFACT_PREFIX = 'artifact:';

function getArtifactsDir(configDir, taskId) {
  return path.join(getTaskDir(configDir, taskId), 'artifacts');
}

function externalizeArtifacts(configDir, taskId, notebook) {
  // Extract inline base64 image data from cells and write to artifacts/ folder.
  // Replaces cell.data with "artifact:filename.png" reference.
  if (!notebook || !Array.isArray(notebook.entries)) return;
  const artifactsDir = getArtifactsDir(configDir, taskId);
  let artifactSeq = 0;
  let dirCreated = false;

  for (const entry of notebook.entries) {
    if (!Array.isArray(entry.cells)) continue;
    for (const cell of entry.cells) {
      if (cell.type !== 'image' || !cell.data) continue;
      // Skip already-externalized cells
      if (cell.data.startsWith(ARTIFACT_PREFIX)) continue;
      // Skip tiny images (not worth externalizing)
      if (cell.data.length < 1000) continue;

      if (!dirCreated) {
        ensureDir(artifactsDir);
        // Count existing artifacts to avoid name collisions
        try {
          const existing = fs.readdirSync(artifactsDir).filter((f) => f.endsWith('.png'));
          artifactSeq = existing.length;
        } catch {}
        dirCreated = true;
      }

      artifactSeq += 1;
      const filename = `plot_${String(artifactSeq).padStart(3, '0')}.png`;
      const filePath = path.join(artifactsDir, filename);

      try {
        const buffer = Buffer.from(cell.data, 'base64');
        fs.writeFileSync(filePath, buffer);
        cell.data = ARTIFACT_PREFIX + filename;
      } catch {
        // If write fails, keep inline data
      }
    }
  }
}

function internalizeArtifacts(configDir, taskId, notebook) {
  // Restore base64 image data from artifacts/ files into cells.
  if (!notebook || !Array.isArray(notebook.entries)) return;
  const artifactsDir = getArtifactsDir(configDir, taskId);

  for (const entry of notebook.entries) {
    if (!Array.isArray(entry.cells)) continue;
    for (const cell of entry.cells) {
      if (cell.type !== 'image' || !cell.data) continue;
      if (!cell.data.startsWith(ARTIFACT_PREFIX)) continue;

      const filename = cell.data.slice(ARTIFACT_PREFIX.length);
      const filePath = path.join(artifactsDir, filename);
      try {
        const buffer = fs.readFileSync(filePath);
        cell.data = buffer.toString('base64');
      } catch {
        // If read fails, leave the reference — the UI will show a broken image
        // which is better than crashing
      }
    }
  }
}

function splitSessionForPersistence(session) {
  // Task identity (task.json) — settings and metadata, rarely changes
  const taskMeta = {
    version: 2,
    taskId: session.taskId,
    taskName: session.taskName,
    parentTaskId: session.parentTaskId || '',
    createdAt: session.createdAt,
    updatedAt: session.updatedAt,
    agentModelId: session.agentModelId || '',
    additionalInstructions: session.additionalInstructions || '',
    bitacora: session.bitacora || '',
    dashboardState: session.dashboardState,
    bootstrap: { done: !!(session.bootstrap && session.bootstrap.done) },
    panelState: session.panelState,
    pendingExternalDrafts: session.pendingExternalDrafts || [],
  };

  // Notebook cells (.svnb) — the permanent record, changes on cell edits
  const notebook = {
    version: 2,
    entries: (session.entries || []).map((entry) => {
      const lean = {
        id: entry.id,
        question: entry.question || '',
        timestamp: entry.timestamp,
        isError: !!entry.isError,
      };
      if (entry.cells) lean.cells = entry.cells;
      if (Array.isArray(entry.executedCells) && entry.executedCells.length > 0) {
        lean.executedCells = entry.executedCells;
      }
      return lean;
    }),
  };

  // Conversation context (.svctx) — volatile LLM state
  const context = {
    version: 2,
    conversationHistory: session.conversationHistory || [],
    sessionSummary: session.sessionSummary || '',
    sessionTokens: session.sessionTokens || 0,
    lastQuestion: session.lastQuestion || '',
    bootstrap: session.bootstrap || { done: false, answer: '', ops: [] },
  };

  return { taskMeta, notebook, context };
}

function mergeSessionFromFiles(taskMeta, notebook, context, configDir, meta) {
  // Restore externalized artifacts (images stored as files instead of inline base64)
  const taskId = (taskMeta && taskMeta.taskId) || (meta && meta.taskId) || '';
  if (notebook && taskId) {
    internalizeArtifacts(configDir, taskId, notebook);
  }
  // Reconstruct the unified in-memory session from the three files
  const merged = {
    ...(taskMeta || {}),
    entries: (notebook && notebook.entries) || [],
    conversationHistory: (context && context.conversationHistory) || [],
    sessionSummary: (context && context.sessionSummary) || '',
    sessionTokens: (context && context.sessionTokens) || 0,
    lastQuestion: (context && context.lastQuestion) || '',
  };
  // Use the full bootstrap from context (has answer + ops), fall back to taskMeta's lean bootstrap
  if (context && context.bootstrap) {
    merged.bootstrap = context.bootstrap;
  }
  return normalizeSession(merged, configDir, meta);
}

function _saveTaskSessionInner(configDir, taskId, session, options = {}) {
  const index = ensureTaskStore(configDir);
  const makeActive = options.makeActive !== false;
  const requestedTaskName = String(
    session.taskName
    || (index.tasks.find((task) => task.id === String(taskId || session.taskId || index.activeTaskId || '')) || {}).name
    || createDefaultTaskName(index.tasks.length)
  );
  const requestedTaskId = String(
    taskId
    || session.taskId
    || index.activeTaskId
    || createTaskId()
  );
  const existing = index.tasks.find((task) => task.id === requestedTaskId) || null;
  const normalized = normalizeSession(session, configDir, {
    taskId: requestedTaskId,
    taskName: session.taskName || (existing && existing.name) || requestedTaskName,
    createdAt: session.createdAt || (existing && existing.createdAt) || new Date().toISOString(),
  });
  normalized.updatedAt = new Date().toISOString();

  // Pre-populate dir name cache so getTaskPath uses the name-based dir, not the raw ID
  const dirName = slugify(normalized.taskName || 'task');
  _dirNameCache.set(`${configDir}:${requestedTaskId}`, dirName);

  // Split and write to three files
  const { taskMeta, notebook, context } = splitSessionForPersistence(normalized);
  externalizeArtifacts(configDir, requestedTaskId, notebook);
  writeJsonAtomic(getTaskMetaPath(configDir, requestedTaskId), taskMeta);
  writeJsonAtomic(getTaskPath(configDir, requestedTaskId), notebook);
  writeJsonAtomic(getTaskContextPath(configDir, requestedTaskId), context);

  upsertTaskMeta(index, normalized);
  if (makeActive) index.activeTaskId = requestedTaskId;
  writeJsonAtomic(getTaskIndexPath(configDir), index);
  invalidateDirNameCache(configDir);
  return normalized;
}

function saveTaskSession(configDir, taskId, session, options = {}) {
  const requestedTaskId = String(
    taskId
    || session.taskId
    || ensureTaskStore(configDir).activeTaskId
    || createTaskId()
  );
  // Pre-populate dir name cache so the lock file path uses the name-based dir
  const taskName = session.taskName || '';
  if (taskName) {
    _dirNameCache.set(`${configDir}:${requestedTaskId}`, slugify(taskName));
  }
  return withFileLockSync(taskLockPath(configDir, requestedTaskId), () => {
    return _saveTaskSessionInner(configDir, taskId, session, options);
  });
}

function saveJaneSession(configDir, session) {
  return saveTaskSession(configDir, session.taskId, session);
}

function updateJaneSession(configDir, updater, options = {}) {
  const resolvedTaskId = options.taskId || null;
  const preIndex = ensureTaskStore(configDir);
  const targetId = resolvedTaskId
    || preIndex.activeTaskId
    || (preIndex.tasks[0] && preIndex.tasks[0].id)
    || '';
  // No tasks exist — nothing to update
  if (!targetId) return null;
  const lockPath = taskLockPath(configDir, targetId);

  const doUpdate = () => {
    const current = loadJaneSession(configDir, resolvedTaskId);
    const next = updater(clone(current)) || current;
    return _saveTaskSessionInner(
      configDir,
      options.taskId || next.taskId || current.taskId,
      next,
      { makeActive: options.makeActive !== false }
    );
  };

  if (lockPath) {
    return withFileLockSync(lockPath, doUpdate);
  }
  return doUpdate();
}

function appendJaneEntry(configDir, entry) {
  return updateJaneSession(configDir, (session) => {
    session.entries.push(normalizeSessionEntry(entry));
    return session;
  });
}

function replaceJaneEntries(configDir, entries) {
  return updateJaneSession(configDir, (session) => {
    session.entries = Array.isArray(entries) ? entries.map(normalizeSessionEntry) : [];
    return session;
  });
}

function setPanelState(configDir, panelState) {
  const index = ensureTaskStore(configDir);
  if (!index.tasks.length) return null;
  return updateJaneSession(configDir, (session) => {
    session.panelState = {
      ...createDefaultPanelState(),
      ...(session.panelState || {}),
      ...(panelState || {}),
      updatedAt: new Date().toISOString(),
    };
    return session;
  });
}

function hasOpenPanelSession(session) {
  return !!(session && session.panelState && session.panelState.open);
}

function enqueueExternalDraft(configDir, draft) {
  return updateJaneSession(configDir, (session) => {
    const normalized = normalizeExternalDraft(draft);
    session.pendingExternalDrafts = Array.isArray(session.pendingExternalDrafts)
      ? session.pendingExternalDrafts.filter((item) => item.id !== normalized.id)
      : [];
    session.pendingExternalDrafts.push(normalized);
    return session;
  });
}

function acknowledgeExternalDrafts(configDir, ids) {
  const idSet = new Set((ids || []).map((id) => String(id)));
  return updateJaneSession(configDir, (session) => {
    session.pendingExternalDrafts = (session.pendingExternalDrafts || []).filter(
      (draft) => !idSet.has(String(draft.id))
    );
    return session;
  });
}

function uniqueTaskName(configDir, requestedName, options = {}) {
  const excludeTaskId = String(options.excludeTaskId || '').trim();
  const tasks = listJaneTasks(configDir).filter((task) => task.id !== excludeTaskId);
  const existingNames = new Set(tasks.map((task) => task.name));
  const base = String(requestedName || '').trim() || createDefaultTaskName(existingNames.size, [...existingNames]);
  if (!existingNames.has(base)) return base;
  let counter = 2;
  while (existingNames.has(`${base} (${counter})`)) counter += 1;
  return `${base} (${counter})`;
}

function transferPanelOpen(configDir, fromTaskId, toTaskId) {
  if (!fromTaskId || fromTaskId === toTaskId) return;
  // Check the task actually exists in the index before trying to load
  const index = ensureTaskStore(configDir);
  if (!index.tasks.some(t => t.id === fromTaskId)) return;
  const current = loadJaneSession(configDir, fromTaskId);
  if (!hasOpenPanelSession(current)) return;
  updateJaneSession(configDir, (session) => {
    session.panelState = {
      ...createDefaultPanelState(),
      ...(session.panelState || {}),
      open: false,
      updatedAt: new Date().toISOString(),
    };
    return session;
  }, { taskId: fromTaskId, makeActive: false });
}

function createJaneTask(configDir, options = {}) {
  const current = loadJaneSession(configDir);
  const { taskId, taskName } = createTaskIdentity(options.name);
  const safeName = uniqueTaskName(configDir, taskName);
  const fresh = createEmptyTaskSession(configDir, {
    taskId,
    taskName: safeName,
    panelOpen: hasOpenPanelSession(current),
    agentModelId: current.agentModelId || '',
    additionalInstructions: current.additionalInstructions || '',
    bitacora: '',  // New tasks start with empty bitácora — fresh line of inquiry
  });
  transferPanelOpen(configDir, current.taskId, taskId);
  const saved = saveTaskSession(configDir, taskId, fresh, { makeActive: true });
  return {
    task: getActiveTask(configDir),
    tasks: listJaneTasks(configDir),
    session: saved,
  };
}

function forkJaneTask(configDir, options = {}) {
  const source = loadJaneSession(configDir, options.sourceTaskId || null);
  const identity = createTaskIdentity(options.name || '');
  const taskId = identity.taskId;
  const taskName = uniqueTaskName(configDir, identity.taskName);
  const now = new Date().toISOString();
  const parentId = String(source.taskId || options.sourceTaskId || '');

  // Build a compressed summary of the parent's conversation as context for the fork.
  // The fork starts with a fresh notebook — no cells copied — but Jane remembers
  // what was discussed and concluded in the parent task.
  const parentSummaryHeader = `[Forked from task "${source.taskName || 'parent'}"]\n`;
  const parentConversation = Array.isArray(source.conversationHistory) ? source.conversationHistory : [];
  const parentSessionSummary = buildConversationSummary(
    parentSummaryHeader + (source.sessionSummary || ''),
    parentConversation
  );

  const forked = normalizeSession({
    // Carry over: settings, dashboard. Bitácora starts fresh — parent context
    // is already captured in sessionSummary.
    dashboardState: clone(source.dashboardState || createDefaultDashboardState()),
    bootstrap: { done: !!(source.bootstrap && source.bootstrap.done), answer: '', ops: [] },
    agentModelId: source.agentModelId || '',
    additionalInstructions: source.additionalInstructions || '',
    bitacora: '',
    // Fresh: empty notebook, empty conversation, parent summary as context
    entries: [],
    conversationHistory: [],
    sessionSummary: parentSessionSummary,
    sessionTokens: 0,
    lastQuestion: '',
    // Fork metadata
    taskId,
    taskName,
    parentTaskId: parentId,
    createdAt: now,
    updatedAt: now,
    pendingExternalDrafts: [],
    panelState: {
      ...createDefaultPanelState(),
      open: hasOpenPanelSession(source),
      updatedAt: now,
    },
  }, configDir, {
    taskId,
    taskName,
    createdAt: now,
  });
  transferPanelOpen(configDir, loadJaneSession(configDir).taskId, taskId);
  const saved = saveTaskSession(configDir, taskId, forked, { makeActive: true });
  return {
    task: getActiveTask(configDir),
    tasks: listJaneTasks(configDir),
    session: saved,
  };
}

function renameJaneTask(configDir, options = {}) {
  const targetId = String(options.taskId || '').trim() || String((getActiveTask(configDir) || {}).id || '');
  if (!targetId) {
    throw new Error('No active Task to rename.');
  }
  const current = loadJaneSession(configDir, targetId);
  const nextName = uniqueTaskName(configDir, options.name || current.taskName || '', {
    excludeTaskId: targetId,
  });

  // Move the task folder/files to match the new name
  const oldDirName = dirNameForTaskId(configDir, targetId);
  const newDirName = slugify(nextName);
  if (oldDirName !== newDirName) {
    const tasksBase = getTasksDir(configDir);
    const oldDir = path.join(tasksBase, oldDirName);
    const newDir = path.join(tasksBase, newDirName);
    if (fs.existsSync(oldDir) && !fs.existsSync(newDir)) {
      // Rename files inside the old directory first
      const oldSlug = slugify(oldDirName);
      const newSlug = slugify(newDirName);
      for (const ext of [TASK_FILE_EXTENSION, TASK_CONTEXT_EXTENSION]) {
        const oldFile = path.join(oldDir, `${oldSlug}${ext}`);
        const newFile = path.join(oldDir, `${newSlug}${ext}`);
        if (fs.existsSync(oldFile)) fs.renameSync(oldFile, newFile);
      }
      // Rename the directory
      fs.renameSync(oldDir, newDir);
    }
  }

  // Update the index with the new dirName before saving
  const index = ensureTaskStore(configDir);
  const taskRecord = index.tasks.find(t => t.id === targetId);
  if (taskRecord) {
    taskRecord.dirName = newDirName;
    taskRecord.name = nextName;
    writeJsonAtomic(getTaskIndexPath(configDir), index);
  }

  const active = getActiveTask(configDir);
  const saved = saveTaskSession(configDir, targetId, {
    ...current,
    taskName: nextName,
  }, {
    makeActive: !!(active && active.id === targetId),
  });
  const task = listJaneTasks(configDir).find((item) => item.id === targetId) || getActiveTask(configDir);
  return {
    task,
    tasks: listJaneTasks(configDir),
    session: saved,
  };
}

function switchJaneTask(configDir, taskId, options = {}) {
  const targetId = String(taskId || '').trim();
  const index = ensureTaskStore(configDir);
  if (!index.tasks.some((task) => task.id === targetId)) {
    throw new Error(`Task not found: ${taskId}`);
  }
  // When updateIndex is false (MCP-originated switch), don't change the
  // global activeTaskId — only load the target session without side effects.
  const updateIndex = options.updateIndex !== false;
  if (!updateIndex) {
    const next = loadJaneSession(configDir, targetId);
    const target = index.tasks.find((t) => t.id === targetId) || null;
    return {
      task: target ? { ...target, active: false, path: getTaskPath(configDir, targetId) } : null,
      tasks: listJaneTasks(configDir),
      session: next,
    };
  }
  const previous = loadJaneSession(configDir);
  const next = loadJaneSession(configDir, targetId);
  const shouldOpen = hasOpenPanelSession(previous) || hasOpenPanelSession(next);
  transferPanelOpen(configDir, previous.taskId, targetId);
  const saved = saveTaskSession(configDir, targetId, {
    ...next,
    panelState: {
      ...createDefaultPanelState(),
      ...(next.panelState || {}),
      open: shouldOpen,
      updatedAt: new Date().toISOString(),
    },
  }, { makeActive: true });
  return {
    task: getActiveTask(configDir),
    tasks: listJaneTasks(configDir),
    session: saved,
  };
}

function deleteJaneTask(configDir, taskId) {
  const targetId = String(taskId || '').trim();
  const index = ensureTaskStore(configDir);
  const idx = index.tasks.findIndex((t) => t.id === targetId);
  if (idx < 0) throw new Error(`Task not found: ${taskId}`);
  // Allow deleting the last task — the project can be empty

  const taskDir = getTaskDir(configDir, targetId);
  try { fs.rmSync(taskDir, { recursive: true, force: true }); } catch {}

  index.tasks.splice(idx, 1);

  if (index.activeTaskId === targetId) {
    index.activeTaskId = index.tasks.length > 0 ? index.tasks[0].id : '';
  }

  writeJsonAtomic(getTaskIndexPath(configDir), index);

  return {
    task: getActiveTask(configDir),
    tasks: listJaneTasks(configDir),
  };
}

function clearJaneSession(configDir) {
  const current = loadJaneSession(configDir);
  const cleared = createEmptyTaskSession(configDir, {
    taskId: current.taskId,
    taskName: current.taskName,
    createdAt: current.createdAt,
    panelOpen: hasOpenPanelSession(current),
  });
  return saveTaskSession(configDir, current.taskId, cleared, { makeActive: true });
}

const { execFile: _execFileCb } = require('child_process');

function silentGitCommit(configDir, message) {
  const tasksDir = getTasksDir(configDir);
  // Fire and forget — never block, never throw, never show output
  try {
    _execFileCb('git', ['add', '.'], { cwd: tasksDir, timeout: 5000 }, (addErr) => {
      if (addErr) return; // not a git repo or git not available — silently skip
      _execFileCb('git', ['commit', '-m', `[selva] ${message}`, '--no-gpg-sign', '--allow-empty'], {
        cwd: tasksDir,
        timeout: 5000,
        env: { ...process.env, GIT_AUTHOR_NAME: 'Selva', GIT_AUTHOR_EMAIL: 'selva@local', GIT_COMMITTER_NAME: 'Selva', GIT_COMMITTER_EMAIL: 'selva@local' },
      }, () => {});
    });
  } catch {
    // git not installed or other system error — silently ignore
  }
}

module.exports = {
  acknowledgeExternalDrafts,
  appendJaneEntry,
  buildConversationSummary,
  clearJaneSession,
  clone,
  createDefaultDashboardState,
  createDefaultJaneSession,
  deleteJaneTask,
  createDefaultPanelState,
  createEntityId,
  createJaneTask,
  enqueueExternalDraft,
  forkJaneTask,
  getActiveTask,
  getSessionPath,
  getTaskIndexPath,
  getTaskPath,
  getTasksDir,
  hasOpenPanelSession,
  listJaneTasks,
  loadJaneSession,
  looksLikePythonExecutionError,
  normalizeNotebookCell,
  normalizePythonRunState,
  normalizeSessionEntry,
  renameJaneTask,
  replaceJaneEntries,
  saveJaneSession,
  sessionIdForConfigDir,
  setPanelState,
  silentGitCommit,
  switchJaneTask,
  updateJaneSession,
  workspaceIdForConfigDir,
};
