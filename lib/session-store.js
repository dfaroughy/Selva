...
function normalizeSession(session, configDir, meta = {}) {
  ...
  const trailId = String(meta.trailId || session.trailId || createEntityId('trail'));
  const createdAt = meta.createdAt || session.createdAt || new Date().toISOString();
  const trailName = String(meta.trailName || session.trailName || 'Trail');
...
function normalizeTrailRecord(record) {
  return {
    id: String(record && record.id ? record.id : createEntityId('trail')),
    name: String(record && record.name ? record.name : 'Trail'),
    file: String(record && record.file ? record.file : ''),
...
function ensureTrailStore(configDir) {
  ...
  if (!existing || !Array.isArray(existing.trails) || existing.trails.length === 0) {
    const legacy = readJsonFile(getLegacySessionPath(configDir));
    const trailId = createEntityId('trail');
    const trailName = createDefaultTrailName(0);
...
function loadJaneSession(configDir, trailId = null) {
  ...
  if (!target) {
    return createEmptyTrailSession(configDir, {
      trailId: createEntityId('trail'),
      trailName: createDefaultTrailName(0),
    });
  }
...
function saveTrailSession(configDir, trailId, session, options = {}) {
  ...
  const requestedTrailId = String(trailId || session.trailId || index.activeTrailId || createEntityId('trail'));
  ...
  const normalized = normalizeSession(session, configDir, {
    ...
    trailName: session.trailName || (existing && existing.name) || createDefaultTrailName(index.trails.length),
